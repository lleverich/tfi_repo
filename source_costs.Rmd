...
advocate = read.csv("4R Advocate Data/2025 Urich Farms/Inputs_machCosts.csv")
```


```{r load calculator sheets}
#Sources 
prices = read_excel("C:/Users/LeannaNigon/The Fertilizer Institute/TFI - Programs/Research/4R Advocate Case Studies/4R Calculator/Leanna's Calculator/FertilizerPrices_Dec2024.xlsx", sheet = 2)
...
df3 <- df2 %>%
  mutate(sourceCost = case_when(
    source == "blend" | source == "starter" | source == "valueAdd" ~ 
...
```{r}
df4 <- df3 %>%
  mutate(microCost = (microDolTon/2000) * sourceRate, 
...
```{r}
# Save csv
write.csv(df6, file = "4R Advocate Data/2025 Urich Farms/Inputs_sourceMachCosts.csv", row.names = FALSE)
```
